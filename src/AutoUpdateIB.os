...
Функция Инициализировать()
	...
	НастройкиОбновления.Вставить("БлокироватьСоединенияИБ", 		  Истина); // Устанавливать блокировку соединений перед обновлением
	НастройкиОбновления.Вставить("СоздаватьРезервнуюКопию", 		  Истина); // Создавать резервную копию
	НастройкиОбновления.Вставить("ВыполнитьОтложенныеОбработчики", 	  Истина); // Выполнить отложенные обработчики обновления
	НастройкиОбновления.Вставить("ВыполнятьСжатиеТаблицИБ", 		  Истина); // Запускать сжатие таблиц информационной базы
	НастройкиОбновления.Вставить("ВосстанавливатьИнформационнуюБазу", Истина); // Использовать восстановление ИБ в случае падения	
...
// Устанавливает значение настройки обновления
	//
	// Параметры:
	//   Ключ	  - Строка - Ключ устанавливаемой настройки 
	//   Значение - Булево - Значение устанавливаемой настройки
	//
Процедура УстановитьНастройкуОбновления(Ключ, Значение) Экспорт
 	
	Если НЕ НастройкиОбновления.Свойство(Ключ) Тогда 
		Лог.Ошибка(НСтр("ru = 'Значение не установлено. Ключ задан неверно.'"));
		ВызватьИсключение "Выполнение обработки прервано";
	КонецЕсли;

	Если НЕ ТипЗнч(Значение) = Тип("Булево") Тогда
		Лог.Ошибка(НСтр("ru = 'Значение не установлено. Переданое значение не является значением типа Булево.'"));
		ВызватьИсключение "Выполнение обработки прервано";
	КонецЕсли;

	НастройкиОбновления[Ключ] = Значение;
	
	Лог.Отладка(СтрШаблон(НСтр("ru = 'Установлена нстройка обновления:
		| %1 - ""%2"".'"), Ключ, Значение));
...
Функция ПолучитьПараметры(Параметр) Экспорт
	...
	ИначеЕсли Параметр = "НастройкиОбновления" Тогда 
...
Функция ОбновитьИнформационнуюБазу(ПараметрыОбновленияКонфигурации, ПараметрыПодключения) Экспорт
	...
	Пока Истина Цикл
		...
		Если Не ВерсияДляОбновления = "КонфигурацияИзменена" Тогда 
			...
			Если Не РезультатВыполнения Тогда
				ЗакончитьОбновлениеИнформационнойБазы(Истина, ПараметрыПодключения, ФайлРезервнойКопии);
				Возврат Ложь;
			КонецЕсли;
	...
	ЗакончитьОбновлениеИнформационнойБазы(Ложь, ПараметрыПодключения, ФайлРезервнойКопии);
	Возврат Истина;
...
// Проверяет наличие обновлений для информационной базы
	...
	//
Функция ПроверитьНаличиеОбновлений(ПараметрыОбновленияКонфигурации, ПараметрыПодключения) Экспорт
...
Функция УстановитьБлокировкуСоединений(ПараметрыПодключения) Экспорт	
	...
	Если НЕ НастройкиОбновления.БлокироватьСоединенияИБ Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция СоздатьРезервнуюКопию(ПараметрыПодключения, ФайлРезервнойКопии) Экспорт
	...
	Если НЕ НастройкиОбновления.СоздаватьРезервнуюКопию Тогда
		Возврат Истина;
	КонецЕсли;
...
// Выполняет все процедуры отложенного обновления информационной базы
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ВыполнитьОтложенныеОбработчикиОбновления(ПараметрыПодключения) Экспорт
	
	Если НЕ НастройкиОбновления.ВыполнитьОтложенныеОбработчики Тогда
		Возврат Истина;
	КонецЕсли;
...
// Загружает файл обновления в информационную базу
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ЗагрузитьФайлОбновленияВИнформационнуюБазу(ПараметрыПодключения, ВерсияДляОбновления) Экспорт
...
// Выполняет обновление конфигурации информационной базы
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ВыполнитьОбновлениеКонфигурацииИнформационнойБазы(ПараметрыПодключения) Экспорт
...
// Выполняет тестирование и исправление
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ВыполнитьТестированиеИИсправление(ПараметрыПодключения) Экспорт
	
	Если НЕ НастройкиОбновления.ВыполнятьСжатиеТаблицИБ Тогда
		Возврат Истина;
	КонецЕсли;
...
// Выполняет неинтерактивное обновление данных информационной базы
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ПринятьОбновленияВИнформационнойБазе(ПараметрыПодключения) Экспорт
...
// Производит завершающие операции с информационной базой после ее обновления
	// 
	// Параметры:
	//  Отказ - Булево - Признак нештатного завершения процедуры обновления;
	//  ПараметрыПодключения - Структура - Параметры подключения к информационной базе (см. в ОбновитьИнформационнуюБазу());
	//  ФайлРезервнойКопии - Строка - Полный путь к файлу резервной копии
	//
Процедура ЗакончитьОбновлениеИнформационнойБазы(Отказ, ПараметрыПодключения, ФайлРезервнойКопии)

	Если Отказ И НастройкиОбновления.ВосстанавливатьИнформационнуюБазу Тогда
...
// Восстанавливает информационную базу из резервной копии
	...
	//  Булево - Признак успешного выполнения.
	//
Функция ВосстановитьИзРезервнойКопии(ПараметрыПодключения, ФайлРезервнойКопии) Экспорт
