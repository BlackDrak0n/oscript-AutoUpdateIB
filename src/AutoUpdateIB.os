...
Функция Инициализировать()
	...
	Каталоги.Вставить("КаталогФайловОбновления", ОбъединитьПути(КаталогAPPDATA, "1C\1Cv8\tmplts\1c")); 
	Каталоги.Вставить("КаталогВременныхФайлов",  ОбъединитьПути(КаталогTEMP, "AutoUpdateIB"));
	Каталоги.Вставить("КаталогРезервныхКопий", 	 ОбъединитьПути(КаталогTEMP, "AutoUpdateIB\DumpIB"));
	...
	ИнтервалыОжидания.Вставить("ЗавершениеРаботыПользователей", 900);
...
//  ВерсияПлатформы 							- Строка - Версия платформы (82, 83).
// 
Процедура ОбновитьИнформационнуюБазу(ПараметрыОбновленияКонфигурации, 
...
// Параметры:
//  ПараметрыПодключения - Структура - Параметры подключения к информационной базе (см. в ОбновитьИнформационнуюБазу());
//  ПолноеИмяФайла - Строка - Полный путь к файлу резервной копии
//
// Возвращаемое значение:
//  Булево - Признак успешного выполнения.
//
